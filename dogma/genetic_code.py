"""
Reference:
    Genetic code data from NCBI
    https://www.ncbi.nlm.nih.gov/Taxonomy/Utils/wprintgc.cgi
"""

from itertools import product

from dogma import (NBCI_GENETIC_CODE_NAMES,
                   NCBI_NUCLEOTIDES,
                   NCBI_CODONS,
                   NCBI_AMINO_ACIDS)
...
class GeneticCode:
    ...
    Parameters
    ----------
    ncbi_id: integer witin NBCI_GENETIC_CODE_NAMES keys, default is 1, standard genetic code
    ...
    def __init__(self,
        ...
        if updated_mappings:
            ...
            assert all([ (len(c) == 3 and isinstance(c, str) ) for c in updated_mappings.keys()])
            assert all([ (len(a) == 1 and isinstance(a, str) ) for a in updated_mappings.values()])
        ...
        if stop_symbol is not '*':
            ...
            updated_amino_acids = updated_amino_acids.replace('*', stop_symbol)
        ...
        # define reversed genetic code, where each amino acid maps to a list of codons
        self.code_reversed = {a: [_ for _ in codons if self.code[_] == a] for a in amino_acids}
    ...
    def update(self, updated_mappings={}, expand=False):
        ...
        expand: Boolean
            ...
            if False, only keys in GeneticCode.code.keys() will be remapped, others silently ignored.
            Default is False

        Usage
        -----
        standard = GeneticCode()  #  default parameter ncbi_id = 1 corresponds to 'Standard' code
        ...
        if not expand:
            updated_mappings = {k:v for k, v in updated_mappings.items() if k in self.code}
    ...
    def __setitem__(self, codon, amino_acid):
        """
        Sets value of code dictionary for specified codon key to specified amino acid.
        ...
        if codon not in self.code:
            raise "Attempting to set codon not currently defined in genetic code. Use update() method to expand."
        else:
            self.code[codon] = amino_acid
    ...
    def __str__(self):
        ...
        return '\n'.join(
            [' '.join([c, a]) for c, a in zip(codons, amino_acids)]
            )


    def __repr__(self):
        """
        Returns string that would evaluate to an equivalent GeneticCode instance.
